...
Supermodeler.prototype._compileSchema = function(schema) {
  ...
  if (schema.methods) {
    if (Array.isArray(schema.methods)) {
      addMethodFromArray(schema.methods);
    } else {
      addMethodFromObject(schema.methods);
    }
  }
  ...
  function addMethodFromArray(methods) {
    methods.forEach(function(method) {
      Ctor.prototype[method.name] = method.function;
    });
  }

  function addMethodFromObject(methods) {
  ...
  function getProperties(props) {
    if (Array.isArray(props)) {
      return getPropertiesFromArray(props);
    } else {
      return getPropertiesFromObject(props);
    }
  }

  function getPropertiesFromArray(propArr) {
    var props = {};
  ...
  function getPropertiesFromObject(propsObj) {
    var props = {}, prop, spec;
